...
def create_viz_link(
    ...
    json_data = get_neuroglancer_json(s3_layer_paths, affine_matrices, output_resolution)
...
def get_neuroglancer_json(s3_layer_paths, affine_matrices, output_resolution):
    ...
    ngl_json['layers'] = [ get_layer_json(i, j, output_resolution) for i,j in zip(s3_layer_paths,affine_matrices) ]
    # print(ngl_json['layers'][0]['source'])
    # print(ngl_json['layers'][1]['source'])
    return ngl_json


def get_output_dimensions_json(output_dimensions):
    # output dimensions must be  in meters
    output_json = copy.deepcopy(minimum_ngl_json['dimensions'])
    output_json['dimensions']['x'][0] = output_dimensions[0]
    output_json['dimensions']['y'][1] = output_dimensions[1]
    output_json['dimensions']['z'][2] = output_dimensions[2]
    return output_json
