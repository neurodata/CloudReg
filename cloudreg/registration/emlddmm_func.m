function [] = register(target_path, target_voxel_size, atlas_path, atlas_voxel_size, parcellation_path, parcellation_voxel_size, registration_prefix, varargin)
% register  Register two images to each other using a modified EM-LDDMM method.
%
%
% 
...
%     addpath([curr_path,'/Functions/spatially_varying_polynomial/'])
    ...
    addRequired(p,'target_path');
    addRequired(p,'target_voxel_size');
    addRequired(p,'atlas_path');
    addRequired(p,'atlas_voxel_size');
    addRequired(p,'parcellation_path');
    addRequired(p,'parcellation_voxel_size');
    % prefix at which to store registration intermediates
    addRequired(p,'registration_prefix');
    %%%% params for preprocessing
    addParameter(p,'missing_data_correction',1);
    addParameter(p,'grid_correction',0);
    addParameter(p,'bias_correction',1);
    %%%% params for registration
    addParameter(p,'fixed_scale',[1.0,1.0,1.0]);
    addParameter(p,'initial_affine',eye(4));
    % smoothness scale (in microns)
    addParameter(p,'a',500);
    % weight of regularization 
    addParameter(p,'sigmaR',1e4);
    % weight of contrast regularization
    addParameter(p,'sigmaC',5.0);
    % total number of iterations
    addParameter(p,'niter',5000);
    % velocity field update step size
    addParameter(p,'eV',1e6);
    % gauss newton affine step size
    addParameter(p,'eA',2e-1);
    % number of timesteps over which to integrate flow
    addParameter(p,'nT',10);
    % order for contrast mapping
    % order 4 means cubic polynomial + constant
    addParameter(p,'order',4);
    
    parse(p,p.Parameters);
    ...
    for downloop = downloop_start : 1
        ...
        for f = 1 : num_slices
            ...
            J_ = double(imread(target_path,f));
            if f == 1
    ...
    save([prefix 'transform_params.mat'],'target_path','parcellation_path','parcellation_voxel_size','parcellation_image_size','output_path_target','output_path_atlas','nxJ0','dxJ0','nxJT','dxJT','dxI','vname','Aname')
    transform_data(target_path,dxJ0,Aname,vname,dxI,parcellation_voxel_size,parcellation_image_size,'atlas',output_path_target,'linear')
