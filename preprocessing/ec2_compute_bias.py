...
def get_list_of_files_to_process(in_bucket_name, prefix):
    s3_client = boto3.client('s3')
    all_files = [f['Key'] for f in tqdm(get_all_s3_objects(s3_client,Bucket=in_bucket_name,Prefix=prefix))]
    return all_files

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--in_bucket_name', help='Name of input S3 bucket where summed tiles live.', type=str)
    parser.add_argument('--in_path', help='S3 path to directory with bias tiles', type=str)
    parser.add_argument('--prefix', help='Prefix for the names of the tiles that need to be summed.', type=str)
    parser.add_argument('--bias_path', help='Full path to output bias file to be saved.', type=str)
    parser.add_argument('--total_tiles', help='total number of tiles', type=float)

    args = parser.parse_args()

    s = time.time()
    all_files = get_list_of_files_to_process(args.in_bucket_name, f'{args.in_path}{args.prefix}')
    num_files  = len(all_files)

    cpu_count = joblib.cpu_count()-1
    start_time = time.time()
    sums = Parallel(cpu_count)(delayed(sum_tiles)(f, args.in_bucket_name) for f in tqdm(chunks(all_files,num_files//(cpu_count))))
    print(f"SUMMING {len(all_files)} tiles took {time.time() - start_time} s")
    sums = [i[:,:,None] for i in sums]
    sum_tile = np.squeeze(np.sum(np.concatenate(sums,axis=2),axis=2))/args.total_tiles
    ...
    print(f"FINAL TILE -- number_of_tiles: {args.total_tiles}")
    ...
    fp.seek(0)
    s3.Object(args.in_bucket_name, args.bias_path).upload_fileobj(fp)
